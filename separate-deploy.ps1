...
heroku config:set JWT_SECRET= --app medical-scheduler-api
heroku config:set HUGGINGFACE_TOKEN= --app medical-scheduler-api
...
heroku config:set REACT_APP_EMAILJS_PUBLIC_KEY= --app medical-scheduler-client
...
heroku config:set REACT_APP_HUGGINGFACE_TOKEN= --app medical-scheduler-client
...
# Install serve dependency for production
npm install --save serve

# Configure Node.js memory limit for frontend app
heroku config:set NODE_OPTIONS="--max_old_space_size=2560" --app medical-scheduler-client

git init
git add .
git commit -m "Deploy frontend to Heroku with memory optimizations"
